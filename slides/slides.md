...
**Sam Dengler**
<br>Distinguished Engineer, Delta Air Lines
...
<img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://samdengler.github.io/ajug-2026-durable-executions/" class="w-28 h-28" />
...
<!--
**Before the talk:**
1. Run `./bin/demo.sh` to start tmux session "ajug"
...
What happens when the service dies mid-execution?

<div class="flex items-center justify-center gap-8 mt-10">

<div class="p-6 border-2 border-blue-400 rounded-lg text-center w-40 relative">
<div class="text-2xl mb-2">Client</div>
<div class="text-sm text-gray-400">curl / httpie</div>
<div class="absolute -bottom-7 left-0 right-0 text-xs text-sky-400 font-medium">1. invoke greeter (async)</div>
<div class="absolute -bottom-12 left-0 right-0 text-xs text-purple-300 font-medium">3. Restate responds</div>
</div>
...
<div class="text-2xl mb-2">Restate Server</div>
<div class="text-sm text-gray-400">localhost:8080</div>
<div class="absolute -bottom-7 left-0 right-0 text-xs text-green-400 font-medium">2. invocation journaled</div>
<div class="absolute -bottom-12 left-0 right-0 text-xs text-sky-400 font-medium">4. trigger handler</div>
</div>

<div class="text-2xl text-gray-500">→</div>

<div class="p-6 border-2 border-red-500 rounded-lg text-center w-52 relative" style="opacity:0.75">
<div class="text-2xl mb-4">Service</div>
<div class="p-3 border border-gray-500 rounded text-sm">
<div class="font-semibold">Handler</div>
<div class="text-gray-400 text-xs mt-1">SDK (ctx)</div>
...
<div class="text-sm text-gray-400 mt-2">localhost:9080</div>
<svg class="absolute inset-0" width="100%" height="100%" style="pointer-events:none">
  <line x1="8%" y1="8%" x2="92%" y2="92%" stroke="#ef4444" stroke-width="4" stroke-linecap="round" />
  <line x1="92%" y1="8%" x2="8%" y2="92%" stroke="#ef4444" stroke-width="4" stroke-linecap="round" />
</svg>
<div class="absolute -bottom-7 left-0 right-0 text-xs text-red-400 font-medium">5. crashes mid-flight</div>
</div>

</div>

<div class="absolute bottom-10 left-0 right-0 text-center text-yellow-400 text-base font-medium">
We kill the service mid-execution. Restate keeps the journal.
</div>
...
```bash
http POST localhost:8080/Greeter/greet/send name=AJUG
```
...
The service comes back - what happens to the in-flight invocation?
...
<div class="p-6 border-2 border-green-400 rounded-lg text-center w-52 relative">
<div class="text-2xl mb-4">Service</div>
<div class="p-3 border border-gray-500 rounded text-sm">
<div class="font-semibold">Handler</div>
<div class="text-gray-400 text-xs mt-1">SDK (ctx)</div>
...
| 4 | **Output** | | `{ result: "You said hi to AJUG!" }` |
...
```bash
# Subscribe: Kafka topic → EventProcessor/process handler
restate subscriptions create \
  kafka://my-cluster/my-topic \
  service://EventProcessor/process
```
...
A virtual object that reschedules itself with a durable delayed send
...
const cronJob = restate.object({
  ...
  handlers: {
    execute: async (ctx: restate.ObjectContext) => {
      ...
      // Schedule the next run with a durable delayed send
      const delay = nextDelay(ctx.date.now()); // from cron expression
      ctx.objectSendClient(cronJob, ctx.key)
        .execute(restate.rpc.sendOpts({ delay }));
    },
  },
});
```

- **Delayed send** - each run schedules the next; no infinite loop
- Each invocation completes, so the journal stays **bounded**
- Survives restarts, node failures, deployments - no external scheduler
...
async (ctx: restate.Context, urls: string[]) => {
  ...
  // Gather results - RestatePromise.all logs completion order (replay-safe)
  const results = await restate.RestatePromise.all(promises);
...
**CLI**
```bash
# Pause / resume a running invocation
restate invocations pause inv_1abc..
restate invocations resume inv_1abc..

# Cancel (rolls back via compensations)
restate invocations cancel inv_1abc..

# Bulk: every invocation for a handler
restate invocations pause Greeter/greet
```
...
**Admin API** (PATCH)
```bash
curl -X PATCH localhost:9070/invocations/inv_.../pause
curl -X PATCH localhost:9070/invocations/inv_.../resume
curl -X PATCH localhost:9070/invocations/inv_.../cancel
```

**Auto-pause**: invocations pause when retries are exhausted - investigate, fix, then resume.
...
const rateLimiter = restate.object({ name: "RateLimiter", handlers: {
  // Short handlers: grab the lock, update, return immediately
  tryAcquire: async (ctx: restate.ObjectContext): Promise<boolean> => {
    const n = (await ctx.get<number>("inflight")) ?? 0;
    if (n >= 10) return false;
    ctx.set("inflight", n + 1);
    return true;
  },
  release: async (ctx: restate.ObjectContext) => {
    const n = (await ctx.get<number>("inflight")) ?? 0;
    ctx.set("inflight", Math.max(0, n - 1));
  },
}});

// Caller does the waiting - the object's lock is never held while sleeping
async (ctx: restate.Context, work: Work) => {
  while (!(await ctx.objectClient(rateLimiter, "global").tryAcquire()))
    await ctx.sleep(1_000); // durable back-pressure
  try { await ctx.run("work", () => doWork(work)); }
  finally { await ctx.objectClient(rateLimiter, "global").release(); }
}
```

- Virtual object = **single writer per key** - no race on the counter
- `tryAcquire` / `release` return immediately - the lock is never held while waiting
- Durable back-pressure lives in the caller's `ctx.sleep()` - callers wait, never dropped

<style>
/* Scoped to this slide: shrink code so the notes below stay on-screen */
.slidev-code { font-size: 0.7rem; line-height: 1.35; }
</style>
